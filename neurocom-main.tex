...
Categorical online learning is an old and well-documented class of learning problems in which (i) a time order is defined on the sequence of input examples  $x_1$, ..., $x_t$, ... with the classifier update taking place after each example presentation and (ii) the output space is categorical, i.e. a single response $y_t$, among $K>1$ possibilities, is expected at trial $t$. In contrast with the offline approach, online learning takes the form of an iterative process, relying on a time-ordered sequence of observations, queries and feedback.   
Two well-known setups obey to this class, i.e. the contextual bandits (see  \cite{lai1985asymptotically} and \cite{auer2002finite}) and supervised online learning (see  \cite{rosenblatt1958perceptron}, \cite{duda1973pattern} and \cite{freund1999large}), the two setups mainly differing by the nature of the feedback. Whether abundant or scarce, deterministic or stochastic, stationary or adversarial, the feedback characteristics critically shape the design of the learning algorithms.  

The case we address in the following is the one-bit feedback in multiclass classification tasks, occasionally called the bandit feedback (see \cite{kakade2008efficient}). This case lies at the crossroad of the online learning and bandit approaches.  A one-bit feedback reflects ``hit-or-miss'' learning situations, in which the single bit of feedback indicates to the learner whether its categorical response was correct (``hit'') or incorrect (``miss''). Wether simple in its principle, the problem is specific to the online approach and was only recently identified and analyzed by the community (see \cite{kakade2008efficient}, \cite{gentile2012multilabel}, etc.). 

{\color{magenta} After a stepwise introduction on the main findings and issues, we present a ...}


{\color{green}
STRUCTURE OF ONLINE LEARNING ALGORITHMS - 

All online learning algorithms share a similar structure, that dates back to the Perceptron \cite{rosenblatt1958perceptron}.  ...
The richer structure of online learning problems was outlined previously by LeCun (NIPS 2003) and XXX (NIPS 2014)
\begin{itemize}
	\item online learning is expected to address the case where data is abundant (many examples, high dimensions, ...)
	\begin{itemize}
		\item sparsity
		\item second order methods?
		\item regularization?
	\end{itemize}
	\item allow quantifying progress
	\begin{itemize}
		\item learning and forget
		\item progrssism vs. conservatism
	\end{itemize}
...
The \textit{decision function} (or politics) $\tilde{y}_t \sim h(x_t)$ embodies both prior information about the regularity of the universe and experience-based information. A reasonable assumption  is to consider that  close-by contexts should provide close-by rewards distributions. This assumption is  implemented through 
linear models that consider, for every categorical response $k$, a linear mapping $w_k$ such that $r(x_t,y_t) \simeq  \langle w_k, x_t\rangle$. This assumption allows (i) to 
settle choice (or action) on reward prediction and (ii) to update models over prediction accuracy.

Multiclass linear models then rely on a set of linear mappings $W = (w_1,...,w_K)$ that can either be considered class-prototypes or class-separatrices. The first case implements a \textit{model-based} approach, and the second a \textit{discriminant-based} approach.
...
In the first case,  second order gradient descent methods such as natural gradient (see \cite{amari2000adaptive}), Gauss-Newton (see \cite{le2004large}), and second order perceptron (see \cite{cesa2005second}) provide almost optimal convergence rates while scaling quadratically with exemplar dimension.
...
In the second case, the quadratic optimization approach (see \cite{anlauf1989adatron}, \cite{crammer2006online}) only provide upper bounds on the error rate but display linear scaling in size and more generally provide a smaller memory footprint. 
...
$K$ linear mappings $w_1, ..., w_K$ are defined and class-decision relies on a best-match politics, i.e.
 $$\hat{y} = \arg \max_{k \in\{1,..,K\}}  \langle w_k, x \rangle$$
 the final choice relying on an $\varepsilon-greedy$ random draw:
...
This is the essence of the linear approach that stipulates that rewards linearly relates to the context, i.e. r 

 The exploration/exploitation trade-off (see \cite ) 

{\color{blue}
So all online learning algorithms share a same structure~:
\begin{itemize}
	\item %the knowledge of the universe relies on a \emph{model}. 
 
	\item \textit{Experiments} (probing) consisting in three parts : first read a context $x_t$ from the environment; second elaborate a response $\tilde{y}_t \in \{1,...,K\}$ and third read feedback $f_t$ from the environment.
	\item A \textit{time index} $t$ organizing individual experiments in a sequence that allow to settle a \textit{disclosure dynamics} with related quantities such as  improvement, cumulated error, regret, memory etc.
	\item An objective mapping over the space of decision functions, that provides directions to improve the decision function. Each new experiment provides additional information, that can be used to update the politics according to the objective.
\end{itemize}
}

{\color{red}
 Third, learning is \textit{active} (see \cite{settles2010active}). Searching information (exploration) is balanced against choosing to exploit the actual knowledge to get rewards (see \cite{auer2003using}).}

{\color{green}
On the one hand, bandit problems consider a universe made of a finite set of arms, numbered from $1$ to $K$, and a gambler. Pulling an arm provides a reward $g$ that may vary among trials and among arms. The objective for the gambler is to maximise the total reward amount, which means exploring the universe to have sufficiently reliable inputs about the expected rewards for each arm. On the other hand, a perceptron relies on  a linear mapping $W$ that is probed against sequential data samples $x_1$, ..., $x_t$, ...   
Every decision $\tilde{y}_t$ is confronted to the real label $y_t$, and the response function is updated so as to avoid mislabelling errors.  }

The behavior of the universe is not fully predictible, and not necessarily stationnary. 
 ...
 and share many common features. First, in both cases, the universe is initially hidden. 
...
%Solutions to the online learning problem include the perceptron (\see{} 


CONSERVATISM VS PROGRESSISM

LA STRUCTURE SEQUENTIELLE DE L'APPRENTISSAGE IMPLIQUE LA NOTION D'UNIVERS, AVEC REPONSE AUX SOLLICITATIONS, MEMOIRE, ACCROISSEMENT DE L'EXPERIENCE, ET POSSIBLEMENT NON-STATIONNARITE

OBJECTIVE, FEEDBACK, REWARD, REGRET ETC... 
 ...
 plays a central role in machine learning, for it embodies, in a simple way, the general question of optimality in decision-making through \emph{active} information seeking. 


We consider here 1-bit feedback online multiclass  learning as part of a more


 Efforts to concile the two approaches are not new, for they 

We first unfold the many interesting aspects and issues of online learning multiclas classification problems. Then in a second part, ... 
...
The structure of \textit{online} multiclass classification problems  is very close to that of contextual bandit problems. 
...
2. ACTION/DECISION IN NON-FULLY DISCLOSED UNIVERSES (ACTIVE LEARNING)

From this large class of online learning problems come profound questions about decision-making in non fully disclosed universes. 
\begin{itemize}
	\item First a temporal order needs to be considered that sets the cadence of a \emph{diclosure dynamics}. This order is described by a time index $t$. At time $t$, the gambler can only use its knowledge from past experiments ($t^\prime < t$) to make its choice. This is the essence of \textit{online} learning.
	\item Second, 
	\item Third, the construction of the model is constrained by choice optimality.  Contrary to a mere information seeking, each choice relies on a balance between information and final gain. Putative additional information may sometimes be sacrified against putative high reward opportunity.  This is the essence of the exploration/exploitation trade-off, but it also draws a clear connection between bandit problems and the active learning framework . In that respect, choice may also be referred as \textit{action} for the actual choice (information vs. reward) plays a significant role in the model update.
	\item  Fourth, a universe is not a mere dataset. The way information is disclosed both depends on individual choices and environment responsiveness to information seeking.  A consequence is that model construction is \textit{dependent} on many factors among which the politics of choice (information seeking against reward) plays a central role and may offer garanties about converging on a satisfying model with respect to a fully disclosed universe (or dataset). 
\end{itemize}  


When decision implies making a choice among $K$ possibilities, a learner needs 
to balance experience (expectations from the past) and curiosity (trying new ways for better achievement).
Different \textit{politics} may be adopted to address this 
 
In the first case, called the ``conservative'' case, the learner's choice illustrates either self-confidence or environment stationarity. In the second case, called the ``progressive'' case, it illustrate either a lack of knowledge or adaptivity to change, such as in the adversarial setting \cite{auer1995gambling},
 or in non-stationary environments (\cite{kivinen2004online}). Balancing self-confidence and adaptivity to change is thus the essence of the exploration/exploitation trade-off.
 ...
 Both stochastic
 (see \cite{auer2003nonstochastic}) and non-stochastic strategies (see \cite{auer2002finite}) can be shown efficient in regulating this delicate balance. 
...
In contrast with the \textit{offline} setting, where the classifiers are obtained through quadratic optimization over a finite set of classified exemples (see \cite{vapnik1998statistical}), no predefinite database is considered under the \emph{online} setting. 
The observation vectors arrive sequentially according to a temporal index $t$ (temporal order). When facing a new example $x_t$, the learner emits a categorical response $\tilde{y}_t \in \{1,... K\}$ and then obtains a feedback $f_t$. The feedback provides an indication about the correctness of the learner's choice. Following \cite{kivinen2004online} and \cite{crammer2006online}, we consider online learning implements a sequential approach to optimization, relying on a specific sequence of observations, decisions and feedbacks.
...
Under  a \emph{relative} scheme (see \cite{crammer2003ultraconservative}), there is no reference value and the linear score of the class-compliant separatrix $\langle w_{y_i}, x_i\rangle$ is expected to be significantly greater than the other linear scores, i.e.
...
Notion de conservatisme vs exhausivité (Anlauf) et progressisme (Kivinen).
