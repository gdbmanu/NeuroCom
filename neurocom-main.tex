...
On contrary to the supervised case, where a full information set $\{\delta(y,1), ..., \delta(y,K)\}$ is disposable, a single bit $\delta(y,\tilde{y})$ is disclosed in our case, the other bits being unset. 
...
The extension of the linear discriminant setup to reproducible kernel Hilbert spaces (RKHS) allows deal with non-linearly separable learning sets at the cost of additional free parameters (specific kernel features). 

Let $\mathcal{K}(.,.)$ a mapping from $ \mathbb{R}^d \times \mathbb{R}^d$ to $ \mathbb{R}^+$ having the Mercer property (see \cite{scholkopf2002learning}). Let $\mathcal{K}(x,.)$ be the projection of example $x$ in $\mathcal{H}$. Then, by construction, a scalar product $\langle.,.\rangle_\mathcal{H}$ in $\mathcal{H}$ is such that: 
 $$\forall w \in \mathcal{H}, \langle w,\mathcal{K}(x,.)\rangle_\mathcal{H} = f(x) $$
...
all previous definitions an results apply.
...
In particular, when considering algorithm \ref{algo:quad} and noting $\alpha_t = \frac {l_{t}}{\mathcal{K}(x_{t},x_{t})+\frac{1}{2C}} (2f_{t} - 1)$, we have:
...
so that, taking definition (\ref{eq:X-kernel}), and setting $\mathcal{T}_{k,t} = \{{t^{\prime}} \leq t: \tilde{y}_{t^{\prime}} = k \text{ and } l_{t^{\prime}} > 0\}$, each 
separatrix $w_{k,t}$ is now defined by a support vectors set 
