...
\begin{algorithmic}
	...
	\STATE Parameters:  $\varepsilon$, $\eta$, $\lambda$
	\STATE Set $W_0 = 0$ (null vector)
	\FOR {each round $t$ = 1,\dots, $T$}
	\STATE Observe $x_t$.
	\STATE Set $\hat{y}_t = \underset{i = 1,\dots,K}{\text{argmax}}\left\langle W_{t-1} ,X_t^i\right\rangle$
	\FORALL {$i \in [1,...,K]$}
	\STATE $p_{i,t}= (1-\varepsilon)\delta(i,\hat{y}_t) + \frac{\varepsilon}{K}$
	\ENDFOR
	\STATE Draw $\tilde{y}_t$ randomly from $p_t = \left(p_{1,t},\dots ,p_{K,t}\right)$.
	\STATE Observe $\delta_t = \delta(y_t,\tilde{y}_t)$.
	\STATE Set $l_t = \left[ 1+(1-2\delta_t)\langle W_{t-1},X_t^{\tilde{y}_t}\rangle\right]_{+}$ 
	\STATE Set $$W_t =  \left\{
	\begin{array}{ll}
	(1-\eta\lambda) W_{t-1} + \eta (2 \delta_t-1) X_t^{\tilde{y}_t} &\text{ if } l_t > 0\\
...
Aside the horizon-based first-order gradient descent presented above, \cite{crammer2006online} consider a quadratic update minimization objective under a class-accuracy linear constraint
$ l_t $.  
...
\begin{algorithm}
\caption{Quadratic optimization}
\begin{algorithmic}
	\STATE $\ \ $
	...
	\STATE Observe $\delta_t = \delta(y_t,\tilde{y}_t)$.
	\STATE Set $l_t = \left[ 1+(1-2\delta_t)\langle W_{t-1},X_t^{\tilde{y}_t}\rangle\right]_{+}$ 
	\STATE Set $$W_t = W_{t-1} + \frac{l_t}{\parallel x_t\parallel^2 + \frac{1}{2C}} (2\delta_t-1) X_t^{\tilde{y}_t}$$
	\ENDFOR
\end{algorithmic}
\end{algorithm}
