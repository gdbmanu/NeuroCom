...
Linear BPA, kernel BPA, kernel SGD and kernel Banditron  are applied to the Segment and Pendigits datasets on figures \ref{pic:PKT} and \ref{pic:PKCM}. 
Online learning with kernels being generally burdened by the increasing size of the prototype vectors set used by the classifier (see \cite{kivinen2004online}), the objective here is mainly to compare the K-BPA native sparsity (algorithm \ref{algo:quad}) to the explicit sparsity control of the SGD method (algorithm \ref{algo:HGD}). The linear BPA provides a baseline reference, while the kernel-Banditron illustrates the upper bound computational costs of a non-sparse update. 
...
The evolving computational cost over time is shown on figure \ref{pic:PKT}. Apart from the linear case, showing a baseline constant cost over time, all kernel-based setups show a monotonic cost rise over time, with the best computational costs obtained for the K-SGD, followed by the K-BPA, and finally the K-Banditron giving an upper bound linear cost rise.  The computational cost of a learning session is consequently $0(T)$ for the linear BPA, $O(T^2)$ for the K-Banditron and in between for the two other cases. The BPA algorithm is expected to reach a constant computational cost after the final number of prototypes is reached, this number being set to 200 on the Segment database and 500  on the Pendigits one. A plateau is roughly observed after 1000 trials on the Segment database, while a continuing rising at slow rate is observed in the other case. The BPA algorithm, while being more costly, show a very close sparsity on the segment database while having a more marked difference on the Pendigits database, showing in general the sparsity effectiveness of the BPA approach.
...
Now turning to the classification rates, the cumulative error rates obtained by linear BPA, kernel BPA, kernel SGD and kernel Banditron on the Segment and Pendigits databases are shown on figure~\ref{pic:PKCM}. %, \ref{pic:PKM}, 
%et \ref{pic:SKCM} 
%et \ref{pic:SKM}  
The first trend to notice is the prevailing of the Kernel setup over the linear one in that case, in particular when considering the Segment database where the linear BPA hardly shows improvement over time. 
 The banditron is generally close to the worst-case scenario while the K-SGD, despite good initial startup, shows difficulty to capitalize improvement after a while (in particular on the Pendigits case). In contrast, the kernel BPA is clearly found to outperform the two other methods, in particular regarding te final error rate. In particular, the final error rate of K-BPA on the Pendigits database is less than 2\%, which is more than 5 time better than the other methods.  This final state-of-the-art classification performances is quite remarkable, given the online setup and partial feedback used here. 
