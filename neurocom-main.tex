...
The result obtained in that case is formally similar to that of  \cite{crammer2006online}. There are however many caveats.

First, the classifier $U$ is composed of $K$ different separatrices, so that its norm is exected to grow linearly with $K$.

Second, the result is  valid for \emph{any sequence} $\tilde{y}_1$, ..., $\tilde{y}_T$, including constant sequences, greedy sequence and pure random sequences. In other words, it is independent from the choice policy.
% and optimal (zero-error) sequence. 
Indeed, whatever the series of responses, only $T$ feedbacks out of $KT$ possible feedbacks are actually read.
%, forming a particular path of actions. 
This sequence probes the environment without exhausting it. The theorem provides a bound on the \emph{observed} cumulative loss, ignoring every unobserved losses. 
...
\begin{itemize}
	\item In case $\tilde{y}_1, ...,\tilde{y}_T$ obey to a uniform random choice (independent from $W$), every separatrix $w^k$ is probed on average the same number of times, providing a non-biased series of class information for large $T$.
	\item In case $\tilde{y}_1, ...,\tilde{y}_T$ obey to a greedy choice, the choice is now dependent on $W_t$. The theorem still garantees  a zero loss after a fixed number of updates. Let us note  $W^*$ this final classifier. Then at least one of the two constraints of \eqref{eq:OVA} is satisfied. Errors occur if $\exists (x,y)$ such that $\langle W^*,X^y\rangle < 1$ and $l_t = 0$. Then, $y$ is not chosen i.e. $\hat{y} \neq y$ and $\langle W^*,X^{\hat{y}}\rangle \leq -1$, i.e. $\forall k, \langle W^*,X^k\rangle \leq -1$.  
	Regarding the second constraint, if $\exists k \neq y$ such that $\langle W^*,X^k\rangle > -1$ and $l_t = 0$, then $k$ is not chosen i.e. $\hat{y} \neq k $ and $\langle W^*,X^{\hat{y}}\rangle > \langle W^*,X^k\rangle > -1$. Then, the only possibility for $l_t = 0$ is  $\langle W^*,X^{\hat{y}}\rangle \geq 1$ and $\hat{y} = y$. There is no possibility for the two constraints being unsatisfied together.
	Then, if the class examples belong to a closed set, then either exists $(x',y)$ such that $\langle w_y^*,x'\rangle > 1$ and then $\exists \rho \in [0,1]$ such that
	$-1 < \langle w_y^*,\rho x + (1-\rho) x'\rangle < 1 $ and $l_t \neq 0$, either 
	 Moreover, if all class examples belong to a closed set, there is no 
\end{itemize}
Finally, an $\varepsilon$-greedy choice, that alternates between uniform sampling and class-effective choice, provides 
