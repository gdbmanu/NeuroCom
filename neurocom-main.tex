...
Contrarily to the supervised case, an \textit{exploration} politics needs to be defined to efficiently sample the decision space. The Banditron adopts a simple  $\varepsilon-greedy$ search allowing the non-reward maximal responses to be occasionally chosen in a uniform way:
...
with the convention $X_t^k = (0, ..., 0,  x_t, 0, .., 0)$ a sequence of null vectors, except at position $k$ where the observation vector $x_t$ is found. The expectation of the update is shown to be that of the multiclass perceptron, and a convergence to the correponding perceptron classifier is obtained, with finite bounds on the cumulative error in linearly separable cases. 

\paragraph{The conservative approach to online classification}
...
Following \cite{kivinen2004online} and \cite{crammer2006online}, we consider online learning implements a sequential approach to optimization, relying on a specific sequence of observations, decisions and feedbacks.
...
The misclassification risk needs to be minimised through local measures of $l_t = l(x_t,\tilde{y}_t,f_t,W_t)$  (the loss function) that estimates the correctness of the current response. The loss function combines the feedback signal $f_t$ with the actual response $\tilde{y}_t$ and the current classifier's parameters $W_t$ to provide additional information about the direction to be followed for future classification improvement (i.e. risk minimization). 
...
\begin{array}{l}
\langle w_k, x \rangle \geq 1 \text{ if } y = k \\
\langle w_k, x \rangle \leq -1 \text{ if }y \neq k.
\end{array}
\right. 
$$
and a corresponding loss function can be the normative multiclass \emph{hinge loss}:  
$$ l(x,W,y) = \sum_{k=1}^K \left[1 + (1 - 2 \delta(y,k)) \langle w_k,x \rangle\right]_+
$$
with $\delta(i,j) =1$ if $i = j$ and 0 elsewhere, and $[u]_+$ equal to $u$ if $u\geq 0$ and 0 elsewhere.
Under  a \emph{relative} scheme (see \cite{crammer2003ultraconservative}), there is no reference value and the linear score of the class-compliant separatrix $\langle w_{y}, x\rangle$ is expected to be significantly greater than the other linear scores, i.e.
$$ \langle w_{y}, x \rangle \geq 1 + \max_{k \neq y} \langle w_k, x \rangle  $$
