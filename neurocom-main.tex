...
\begin{abstract}
%% Text of abstract
The bandit classification problem considers learning the label of unknown samples using only one bit of feedback  at each trial.
A sparse and lightweight solution to this problem is proposed, using a straightforward reduction of the OVA (``one-versus-all'') hinge loss setup. The issued sequential norm-minimal update solves the classification problem in finite time in the separable case, provided enough redundancy is present in the data. An $O(\sqrt{T})$ regret in moreover expected in the non-separable case.
...
Contextual bandit \sep Hinge loss \sep Online learning \sep Kernel methods 
...
We adapt in this paper a similar view to the bandit feedback case, where online learning relies on a local norm minimization under standard linear-convex constraints. Our conservative approach allows to determine similar regret bounds than the original paper of \cite{crammer2006online}, providing strong guaranties on  convergence capabilities, that are confronted to other methods over synthetic and real datasets.  
