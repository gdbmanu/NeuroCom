...
In the specific kernel case (Pendigit and Segment databases), the non-separability assumption allows to withdraw the stiffness parameter, i.e. to consider $C \rightarrow \infty$. The Kernel function used in simulations is the Laplacian kernel, i.e.~:
...
\begin{figure}[htp]
	...
	\caption{Perceptron, Passive-Aggressive (PA), Banditron, Confidit and one-Bit feedback Passive Aggressive (BPA) cumulative errors  over time for the SynSep (9 classes), SynNonSep (9 classes) and Reuters (53 classes) databases. Parameters are in table 2.}
	\label{pic:BPASS}
\end{figure}
...
Moreover, the marked prevailing of BPA over Confidit is also noticeable for its algorithmic complexity is far lesser \footnote{Confidit uses a second order sample covariance estimates, this covariance matrix being reduced  to a mere diagonal in the high-dimensional case (see  \cite{crammer2013multiclass}).}. 
...
\caption{Linear BPA (algorithm 1), Laplace kernel BPA (algorithm 1), Laplace kernel SGD (algorithm 2)  and Laplace kernel Banditron average computation times over instance number on Pendigit (top) and Segment (bottom) databases. Parameters are in table 2.}
\label{pic:PKT}	
\end{figure}

Linear BPA, kernel BPA, kernel SGD and kernel Banditron  are applied to the Pendigits and Segment datasets on figures \ref{pic:PKT} and \ref{pic:PKCM}. 
Online learning with kernels being generally burdened by the increasing size of the prototype vectors set used in the classifier function (see \cite{kivinen2004online}), the objective here is mainly to compare the K-BPA native sparsity (algorithm \ref{algo:quad}) to the explicit sparsity control of the SGD method (algorithm \ref{algo:HGD}). The linear BPA provides a baseline reference, while the kernel-Banditron illustrates the upper bound computational costs of a non-sparse update. 
...
The evolving computational cost over time is shown on figure \ref{pic:PKT}.
...
\begin{figure}[htp]
	...
	\caption{Linear BPA (algorithm 1), Laplace kernel BPA (algorithm 1), Laplace kernel SGD (algorithm 2)  and Laplace kernel Banditron cumulative errors over time on Pendigits (top) and Segment (bottom)  databases. Parameters are in table 2.}
	\label{pic:PKCM}
\end{figure}
