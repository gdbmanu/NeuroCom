...
\begin{itemize}
	...
	\item In case $\tilde{y}_1, ...,\tilde{y}_T$ obey to a greedy choice, the choice is now dependent on $W_t$. The theorem still guarantees a zero loss after a fixed number of updates. Let us note  $W^*$ this final classifier. Then at least one of the two constraints of \eqref{eq:OVA} is satisfied. 
	\begin{itemize}
	...
	Let us consider this case (i.e. $\exists (x,y)$ such that $\langle w_y^*,x\rangle \leq -1$). If $\exists (x',y)$ such that $\langle w_y^*,x'\rangle > 1$. All class examples belong (by construction) to a convex set (defined by $U$), so that $\exists \rho \in [0,1]$ such that
	$-1 < \langle w_y^*,\rho x + (1-\rho) x'\rangle < 1 $ which brings out a non-zero loss, in contradiction with the theorem.
