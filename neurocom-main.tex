...
Different algorithms are evaluated on two synthetic datasets and three real-world datasets, in order to test different aspects of the multiclass  setting. Their principal characteristics are provided in table ~\ref{table:mce}.

\begin{table}[h]
	\caption{Five datasets features, with $n$ the number of instances, $d$ the dimension of vectors and $K$ the number of labels.}
	\label{table:mce}
	\begin{center}
...
The two first datasets mimick text documents vector encoding, with a small-vocabulary of dimension 400. $10^5$ instances, belonging to 9 different classes, are generated. The detailed construction of the two datasets is given in \cite{kakade2008efficient}. In the first dataset, called  "SynSep", the different classes are linearly separable. In the second dataset, called "SynNonSep", a 5\% label noise is introduced, rendering the dataset non separable.  
...
The fourth dataset, named "Pendigit", was created by \cite{alimoglu1996combining}. It owns 7494 instances, with 16 features per instance and 10 classes. 
The fifth dataset, named  "Segment" \cite{Lichman:2013}, owns 2310 instances. Each feature vector is build from 3x3 pixels excerpts from natural images, with 19 features per instance and 7 classes. 
