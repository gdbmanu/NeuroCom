...
\title{Sparse online learning with bandit feedback}
...
\author[centralif]{Hongliang Zhong}
\author[ins]{Emmanuel Dauc\'e \corref{cor1}}

\address[centralif]{Aix Marseille Univ, CNRS, Centrale Marseille, LIF, Laboratoire d'Informatique Fondamentale, Marseille, France}
\address[ins]{Aix Marseille Univ, Inserm, INS, Institut de Neurosciences des Systèmes, Marseille, France}

\cortext[cor1]{Corresponding author}

\begin{abstract}
%% Text of abstract
The bandit classification problem considers learning the labels of a time-indexed data stream under a mere ``hit-or-miss'' binary guiding.
Adapting the OVA (``one-versus-all'') hinge loss setup, we develop a sparse and lightweight solution to this problem. The issued sequential norm-minimal update solves the classification problem in finite time in the separable case, provided enough redundancy is present in the data. An $O(\sqrt{T})$ regret in moreover expected in the non-separable case.
The algorithm shows effectiveness on both large scale text-mining and machine learning datasets, with ($i$) a favorable comparison with the more demanding confidence-based second-order bandits setups on large scale datasets and ($ii$) a good sparsity and efficacy when a kernel approach is applied to non-separable datasets.
%The non-stationary and adversarial setting  may be treated with horizon-based approaches, as proposed by \cite{kivinen2004online}, for it  shows both effectiveness, sparsity and adaptivity in  simulations.  
\end{abstract}

\begin{keyword}
%% keywords here, in the form: keyword \sep keyword
Contextual bandits \sep Hinge loss \sep Online learning \sep Kernel methods 
...
We adapt in this paper a similar view to the bandit feedback case, where online learning relies on a local norm minimization under standard linear-convex constraints. Our conservative approach allows to determine similar regret bounds than the original paper of \cite{crammer2006online}, providing strong guaranties on  convergence capabilities, that are confronted to other methods over synthetic and real datasets\footnote{  
This work was partly presented at the ECML-PKDD doctoral consortium (see \cite{zhong2015b}). Substantial refinements and new notations are used in the present paper. The kernel extension, with corresponding developments and simulations, were not present in the conference article.}.   
...
\subsection{One-bit bandit feedback}
...
where classifiers are updated through local measures of a hinge loss function: 
$$l_t = l(x_t,W_{t-1},y_t)$$


Different hinge loss functions and corresponding margin constraints can be defined depending on the task and feedback characteristics. 
...
The normative setup puts additional constraints on the classification task (see \cite{crammer2003ultraconservative}), but in counterpart provides mapping-independence across the different classes.
 ...
 %
 %Contrarily to a mere reward (or penalty), a 
...
The ability to store only the most relevant observation vectors is however a critical property of the discriminant approach we try to preserve in the  bandit  setup considered here. 
...
Adapting the one-vs-all normative margin constraint (presented in eqs.(\ref{eq:OVA-A}-\ref{eq:OVA-B})) to the bandit  case implies consider now the loss function:
...
\begin{algorithm}[t!]
	\caption{Bandit Passive-Aggressive (BPA)}\label{algo:quad}
	\begin{algorithmic}
...
%In the setup we consider, the loss $l_t$ relies on the current choice (or action) $\tilde{y}_t$. The loss exactly probes how good or bad the current choice is, without telling how good or bad other choices would be. 
...
The result obtained in that case is formally similar, and even slightly tighter\footnote{using a relative hinge-loss, the multiclass upper bound is $4R^2 \parallel{U}\parallel^2$.}, than the one obtained by \cite{crammer2006online} in the multiclass case. Note however that the reference classifier $U$ being defined in $\mathbb{R}^{Kd}$, while observation vectors are in $\mathbb{R}^{d}$, a linear dependence on the label set cardinality $K$ is to be expected. 
...
%Two important cases need here to be examined:
	...
	\paragraph{Random uniform  policy} If $\tilde{y}_1, ...,\tilde{y}_T$ obey to a uniform random choice (independent from $W$), i.e.
...
This bound is once again similar to that obtained by 
...
classifier $U$ (including that of a loss-minimizer classifier $U^*$). In approximation\footnote{If the loss variance is small, i.e. $\langle l_t^2\rangle_t \simeq \langle l_t \rangle_t^2$.},  the  final loss is thus expected to be on average twice that of the best classifier. As shown in  \cite{crammer2006online}), this error bound is less tight than that of \cite{freund1997decision}, and additional regularization (with finite $C$) is needed to attain $O(\sqrt{T})$ regret,  
we do not develop here for brevity. 

Given the aggressiveness of the algorithm, this ``loss-doubling'' can be interpreted the following way: in the long run, each non-zero loss  encountered is expected to provoke an aggressive (overfitting) update, that will need on average a same amount of loss in subsequent rounds to recover. When the proportion of outliers is not to strong, the separability assumption can be seen as a boundary condition to which the classifier periodically returns between temporary excursions throughout disrupting updates and recovery. 

Like previously, this result is not a strict bound on the classification error, for a mere sample of the total OVA losses is visited in one run.
The separable case is now the boundary condition toward which the classifier continually returns, for which specific policies need to be assumed to provide effective final classification (see previous section). 
...
\begin{table}[h]
	\caption{Parameters setting for different algorithms and different datasets.  {\bf P} stands for Perceptron, {\bf PA} for Passive Agressive, {\bf B} for Banditron, {\bf C} for Confidit, {\bf BPA} for the Bandit Passive Aggressive (algorithm 1), {\bf K-B} for the kernel Banditron, {\bf K-BPA}  for the kernel  realization of BPA (algorithm 1) and {\bf K-SGD} for the kernel  realization of SGD (algorithm 2).}
	\label{table:bpa}
	%\begin{center}
...
\begin{itemize}
	...
	\item The Banditron (\cite{kakade2008efficient}), Confidit (\cite{crammer2013multiclass}), and our algorithms BPA (algorithm \ref{algo:quad}) and HGD (algorithm \ref{algo:HGD}) only have a one-bit feedback at disposal.
\end{itemize} 
...
\begin{figure}[htp]
	...
	\caption{Banditron and Bandit Passive Aggressive (BPA) final error rate in function of the exploration rate $\varepsilon$, on SynNonSep (top) and Reuters (bottom) datasets. Parameters are in table 2.}
	\label{pic:BPASNSerr}
%\end{figure}
...
the effect of the exploration parameter $\varepsilon$ on the final classification rate, for the Banditron and the Bandit Passive-Aggressive (algorithm \ref{algo:quad}). Apart from better final classification rate, the passive aggressive setup is shown insensitive to variable $\varepsilon$, on contrary to the Banditron having a known exploration rate dependence. Greedy exploitation and pure exploration show similar effectiveness in our case. In practice, this result allows consider decreasing exploration rates over learning sessions, for to take advantage of the classification rates attained in the course of learning. 
...
\begin{figure}[htp]
	...
	\caption{Perceptron, Passive-Aggressive (PA), Banditron, Confidit and Bandit Passive Aggressive (BPA) cumulative errors  over trial number for the SynSep (9 classes), SynNonSep (9 classes) and Reuters (53 classes) databases. Parameters are in table 2.}
	\label{pic:BPASS}
\end{figure}

The Perceptron, Passive-Aggressive (PA), Banditron, Confidit and Bandit Passive Aggressive (BPA) cumulative errors are compared on figure \ref{pic:BPASS} over the SynSep, SynNonSep and Reuters RCV1-v2 datasets. 
...
We have shown that a conservative reduction of the OVA hinge-loss  provides an effective and lightweight solutions to the  bandit classification problem (as defined by \cite{kakade2008efficient}). 
In particular, when adapting the passive aggressive setup proposed in the supervised case by \cite{crammer2006online}, 
...
Here, however, the bound is not an upper bound of the classification error. Additional regularity assumptions, such as observation sets convexity, or a partly uniform sampling of the label space, need to be considered to provide comparable  upper error bounds. In addition, as in \cite{crammer2006online}, a soft margin stiffness parameter $C$ needs to be optimized to reach $O(\sqrt{T})$ regret in the stationary case.  

%SUBSAMPLING OF AN OVERCONSTRAINED PROBLEM. NOt ALL CONSTRAINTS ARE SOLVED, BUT NONETHELESS ENOUGH CONSTRAINTS ARE SATISFIED FOR A CORRECT CLASSIFICATION TO BE REACHED IN THE LONG RUN. 
...
%EXPECTED / NOT EXPECTED? CONSEQUENCES?
...
%RESILIENCE TO LABEL NOISE IS CONTRADICTORY WITH AGGRESSIVENESS


%While most contextual bandit setups concentrate to large-scale text-mining and recommendation datasets, 
...
In conclusion, our approach  provides many practical advantages when considering the bandit classification problem, among which ($i$) simplicity, with easy algorithmic implementation, ($ii$) linear scaling in space, ($iii$) sparsity, ($iv$) linear scaling to the labels space cardinality, 
...
($v$) kernel compatibility and ($vi$) resilience to label noise.
It moreover reveals surprisingly effective when compared to the more elaborate noise-aware UCB-like setups.
This is, to our best knowledge, the first adaptation of sparse online learning principles to the bandit case, providing avenue toward effective kernel implementations of  non-linearly separable contextual bandit setups.   %One possible reason is that soft margins are as effective to deal with   incorrectly labelled samples, which is at odd with many real world situations having to deal with label noise, and more generally adversarial settings. 
%Although the approach does not provide second-order estimates, it may allow implement simple UCB policies. 
%XXX resilience to label noise
Our OVA reduction may even generalize to slightly more demanding tasks, like the multi-label setup generally considered in recommender systems. Complement investigations are however needed to address both the non-stationary and adversarial cases.
The H-horizon window approach, as proposed by \cite{kivinen2004online}, may in some cases be substituted, for it  shows both effectiveness, sparsity and adaptivity in simulations. 

%EPSILON EXPLORATION RATE PROVIDES A LINEAR REGRET. THIS POINT IS TO
%BE CONSIDERED WITH CARE.

%ALTERNATIVE POLICIES? UCB? PURE GREEDY?

%PLUSIEURS PARADOXES EXPLIQUENT LE RETARD DANS L'UTILISATION D'OVA :

%NOISE RESILIENCE IS UNEXPECTED. OUR APPROACH IS AT ODD TO THE NOISE-AWARE CONFIDENCE-BASED METHODS. IT MAY RESULT BOTH FROM THE CONSERVATIVENESS AND 

%AUTRE PARADOXE : OVA EST PREFERABLE (PLUS SPARSE) ALORS QUE C'EST LE CONTRAIRE (RELATIF) QUI EST SPARSE DANS LE CAS SUPERVISE

%HEAVILY RELIES ON C PARAMETER (cross validation) WHICH SHOULD BE SPECIFIC TO THE DATASET. 
%HOW THE SETUP WOULD BEHAVE IN MORE ADVERSARIAL SETTINGS IS UNKNOWN. 

%OUR APPROACH IS TIGHTLY LINKED TO ONLINE BINARY SUPERVISION. DIFFERENT UPDATES MAY BE CONSIDERED, LIKE THE PERCPTRON UPDATE, LOGISTIC REGRESSION, MULTI-LAYERED PERCEPTRON ETC. THERE Is HOWEVER A STRONG ROLE OF 'ONE-SHOT'-AGGRESSIVENESS AND IMMEDIATE MARGIN CONSTRAINT SATISFACTION THAT RENDERS HE METHOD INSENSITIVE TO CLASS IMBALANCE THAT  BECOME MORE AND MORE CRITICAL AS THE NUMBER OF LABELS INCREASES. 

%OUR PARADOXICAL RESULT IN THE SEPARABLE CASE SEEMS TO RELY ON THE GREEDINESS AND AGGRESSIVENESS OF THE PA SETUP THAT, IN PRINCIPLE, RENDERS THE METHOD SENSIBLE TO LABEL NOISE.


%The many advantages developped here should allow reconsider this point. 

%The reason why quadratic was not considered up to now relies mainly on those two aspects : linear regret in practice and unfitted to the adversarial case.


%Here translated into a reduced OVA hinge loss, we  show that sequentially fixing the series of observed losses allow convergence toward an adapted classifier. 
%This budget is here used to settle a single margin constraint under the OVA setup.  margin constraints is then expected to solve in approximation the total (unobserved) set of margin constraints, provided enough redundancy is present in the data. 
%This approach is well fitted to the specific stationary classification tasks considered here, and for that reason is shown to powerfully compete with the more demanding confidence-based second-order bandits setups. 



From a more general standpoint, the bandit classification problem implements a form of active learning in scarcely labeled environments, with a limited (1 bit) information budget at each round. 
Apart for text mining and recommender systems, it would  probably need additional developments to reach full relevance in real-world problems having both a sequential organization and undergoing a strict ``win-or-loose'' return.
% than the mere classification problems covered here. 
%It is still difficult to figure out the exact span of problems it may address.
In the case of artificial games, for instance, both multiple moves and temporal credit assignment (see \cite{sutton1998reinforcement}) would need to be considered in addition.    
Problems implying non-vectorial spaces (like graph spaces) and corresponding similarity metrics may also be addressed, just like it is the case for other margin-based risk-minimizer techniques (see \cite{chen2009similarity}).
