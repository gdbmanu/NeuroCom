...
Online learning is a \textit{process} in which an agent (the ``learner'') probes its environment sequentially to obtain \textit{information} that is eventually used to improve its \textit{fitness}.
...
To every different context may correspond a different distribution of rewards. Most of the time, however, a metric exists in the context space so that close-by contexts are expected to provide close-by rewards distributions.  
...
On the other hand, the quadratic optimization approach (see \cite{anlauf1989adatron}, \cite{crammer2006online}) only provide upper bounds on the error rate but display linear scaling in size and more generally provide a smaller memory footprint. 
