...
Online supervised classification schemes (see \cite{freund1997decision}, \cite{cesa2005second} or \cite{crammer2006online}) show striking similarities with the contextual bandit setup. Online learning means updating the classifier at each trial, i.e. on the one hand using past experience to classify the current instance $x$, but also, on the other hand, using additional knowledge provided by the new label $y$ to improve future responses. Most supervised learning schemes do not implement curiosity however, for the label values are non stochastic and best matching label provably the optimal choice. 
