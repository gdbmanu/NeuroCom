...
The case we address in the following is the one-bit feedback in multiclass classification tasks, occasionally called the bandit feedback classification problem (see \cite{kakade2008efficient}), for the outcome is either 0 (failure) or 1 (success). This case lies at the crossroad of the supervised online learning setup and the bandit setup.  A one-bit bandit feedback reflects a ``hit-or-miss'' learning situations, in which a single bit indicates to the learner whether its categorical response was correct (``hit'') or incorrect (``miss''). Wether simple in its principle, this problem is bound to the online approach and was only recently addressed by the community (see \cite{kakade2008efficient}, \cite{crammer2013multiclass}, etc.). 
...
The separable case is now the boundary condition toward which the classifier continually returns, for which specific policies need to be assumed to provide effective convergence guarantees (see previous section). 
...
Here, however, the bound is not an upper bound of the classification error. Additional regularity assumptions, such as observation sets convexity, or a partly uniform sampling of the label space, need to be considered to provide comparable error upper bounds. In addition, as in \cite{crammer2006online}, a misclassification stiffness parameter $C$ needs to be optimized to reach $O(\sqrt{T})$ regret in the stationary case.  
...
In conclusion, our approach is found to provide many practical advantages, among which:
\begin{enumerate}
	\item simplicity, with easy numerical implementation
	\item linear scaling in space
	\item sparsity, each observation vector being stuck to one class-separatrix only, 
	\item linear scaling to the labels space cardinality, 
...
From a more general standpoint, the bandit classification problem implements a form of active learning in scarcely labeled environments, with a limited (1 bit) information budget at each round. We here translated this information into a reduced OVA hinge loss, and shown that sequentially fixing the series of observed losses allow the convergence toward an adapted classifier. 
...
It appears quite surprising the sparse online approach to the contextual bandit problem was not much considered up to now in the machine learning community. One possible reason is that a sparse setup is generally prone to overfit incorrectly labelled samples, which is at odd with many real world situations having to deal with label noise, and more generally adversarial settings. Moreover, the approach does not provide second-order estimates that may allow implement UCB, and the convergence to a correct classifier was seemingly not clearly established up to now.  

XXX resilience to label noise
    
Apart from text mining and recommender systems it naturally addresses, the one-bit bandit classification setup would  probably need additional developments to reach full relevance in real-world problems obeying to a sequential organization and undergoing a strict ``win-or-loose'' return.
