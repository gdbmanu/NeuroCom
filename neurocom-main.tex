...
\begin{itemize}
	...
	\item  Compliant with the one-vs-all (OVA) construction, the \emph{normative} margin setup imposes the classifier to provide a response that overtakes in norm a reference value $a$. Taking  $a = 1$ for reference, it tells $\forall k$: \begin{align}%\label{eq:OVA}
...
{\color{blue} This "agnostic" approach to non-explicitely disclosed labels %avoids weight coefficients asymmetry in the label hit case and 
...
Indeed, whatever the  $ (\tilde{y}_1$, ..., $\tilde{y}_T)$ sequence, only $T$ feedbacks signals out of $KT$ potential feedbacks are actually read,
...
\paragraph{} If we turn now to an arbitrary classifier $U$, without separability assumption, the following theorem holds~:
...
%\paragraph{}
\begin{algorithm}[t!]
	\caption{H-horizon gradient descent}\label{algo:HGD}
	\begin{algorithmic}
...
separatrix $w_{k,t}$ is now defined by a set of reference vectors 
$\{x_{t^{\prime}}\}_{{t^{\prime}}\in \mathcal{T}_{k,t}}$ 
so that 
	$$w_{k,t} = \sum_{t^{\prime}\in \mathcal{T}_{k,t}} \alpha_{t^{\prime}} \mathcal{K}(x_{t^{\prime}},.)$$

The total number of reference vectors is incremented each time a non-zero loss is carried out. From theorem \ref{theo:BPAT1}, we know this number is bounded since any dataset is separable in the redescription space. 
...
The two first datasets mimick text documents vector encoding, with a small 400-dimensional vocabulary. $10^5$ instances, belonging to 9 different classes, are generated. The detailed construction of the two datasets is given in \cite{kakade2008efficient}. In the first dataset, called  \textit{SynSep}, the different classes are linearly separable. In the second dataset, called \textit{SynNonSep}, a 5\% label noise is introduced, rendering the dataset non separable.  

The third dataset comes from the Reuters \textit{RCV1-v2} collection (see \cite{David04RCV}). This dataset is typical of text mining applications, containing high dimension vectors (47,236 vocabulary entries), and a large number of instances ($10^5$). Those numbers allow to test the algorithms capability to deal with real-world scaling. 
...
The two last datasets are real-world datasets, having a smaller number of instances and a smaller dimension, allowing to test the algorithms performance and sparsity in Kernel embedding case.   
...
\begin{table}[h]
	\caption{Parameters setting for different algorithms and different datasets.  {\bf P} stands for Perceptron, {\bf PA} for Passive Agressive, {\bf B} for Banditron, {\bf C} for Confidit, {\bf KB} for the Kernel Banditron, {\bf K-Algo1} and {\bf K-Algo2} for the Kernel  realization of algorithms 1 and 2.}
	\label{table:bpa}
	%\begin{center}
	...
	\begin{center}
		\begin{tabular}{llllll}
			...
			\hline
			{\bf Dataset}  & {\bf P} & {\bf PA } & {\bf B}& {\bf C} & {\bf Algo1}\\
			\hline
			...
			&&&&& $C = 10^{-2}$\\
			\hline
			&{\bf KB} & {\bf Algo1} & {\bf K-Algo1} &{\bf K-Algo2}\\
			\hline
			Pendigit & $\sigma = 10$ & $\varepsilon = 0.3$ &$\varepsilon = 0.3$&$\sigma = 10$\\
			&$\varepsilon =0.1$&& $\sigma = 10$ & $H = 500$\\
			Segment & $\sigma = 1$ & $\varepsilon = 0.3$ & $\varepsilon = 0.3$ &$\sigma = 1$\\
			&$\varepsilon =0.1$&&$\sigma = 1$ & $H = 200$\\
			%LR(26 letters) & null &  $C=0.1$ & $\varepsilon = 0.2$& $\eta=10^2$ & $\varepsilon = 0.8,C= 1$ \\
...
Only the online learning setup is here considered. For a given dataset, each instance is only presented once. The classifier update starts right after the first instance presentation, and finishes at the last one. For each instance, a single response is carried out, and a single corresponding feedback is obtained.
...
In order to faithfully compare the methods, all parameters are obtained by cross-validation on the final classification rate. The resulting parameters used in simulation are given in table \ref{table:bpa}. 
...
\paragraph{Remark} Both Banditron and Algorithm \ref{algo:quad} use an $\varepsilon$-greedy exploration policy, that has a direct effect on the  classification rate, for the actual response  $\tilde{y}_t$ is differents from $\hat{y}_t$ (in a proportion equal to $\varepsilon$). A residual exploration error thus necessarily exists, whatever effective the classifier $W$ at separating the data. In order to properly compare algorithms, we need to evenly evaluate improvement at different $\varepsilon$, i.e. only use the internal noise-free estimate $\hat{y}_t$ for algorithm evaluation.
