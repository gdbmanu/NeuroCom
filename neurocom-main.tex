...
Following   \cite{kivinen2004online} and \cite{crammer2006online}, we consider that online learning implements a sequential approach to risk minimization, relying on a specific sequence of observations, decisions and feedbacks.
...
Under  a \emph{relative} margin setup (see \cite{crammer2003ultraconservative}), \emph{aka} ``Winner-Takes-All'' (WTA), there is no absolute reference value but instead a distance reference, so that the linear score of the class-compliant separatrix $\langle w^y, x\rangle$ is expected to overtake  the other linear scores by at least $a$, i.e. (taking $a=1$)
