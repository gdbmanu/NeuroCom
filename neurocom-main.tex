...
Online multiclass learning problems constitute a specific class of learning problems in which (i) a time order is defined on the sequence of input examples  $x_1$, ..., $x_t$, ... with the classifier update taking place after each example presentation and (ii) the output space is categorical, i.e. a single response $y_t$, among $K>1$ possibilities, is expected at trial $t$.  Online multiclass learning encompass two well-documented classes of learning problems, i.e. the contextual bandits (see \cite{lai1985asymptotically} and \cite{auer2002finite}) and supervised online learning (see  \cite{duda1973pattern} and \cite{freund1999large}), the two classes of problems mainly differing by the nature of the feedback. Whether abundant or scarce, deterministic or stochastic, stationary or adversarial, the feedback characteristics critically shape the design of the learning algorithms.  The case we will address in the following is the one-bit feedback, occasionally called bandit feedback (see \cite{kakade2008efficient}), that is a prototypical case lying at the crossroad of the two approaches.  
...
A one-bit feedback, sometimes called bandit feedback in multiclass classification (see \cite{kakade2008efficient}), reflects ``hit-or-miss'' learning situations, in which a single bit feedback information indicates to the learner whether its categorical response was correct (``hit'') or incorrect (``miss'').
...
A bandit problem (see \cite{robbins1952bandit}) is composed of a universe made of a finite set of arms, numbered from $1$ to $K$, and a gambler. The behavior of the universe is not fully predictible, and not necessarily stationnary. Pulling an arm will provide a reward $g$ that may vary among trials and among arms. The objective for the gambler is to maximise the total amount, which means exploring the universe to have sufficiently reliable inputs about the expected rewards for each arm. Solving the problem thus means both optimizing the final gain, but also \emph{learning the universe from experience}. 


%Les problèmes de bandit simples se généralisent au cas des bandits dits contextuels \cite{langford2008epoch}. 
Contextual bandit problems (see \cite{lai1985asymptotically} and \cite{auer2002finite}) provide sensibly richer universes with distributions of rewards biased by a context $x$ that is given to the gambler before decision making. The universe is now made of couples (context, rewards distribution). To every different context may correspond a different distribution of rewards. Most of the time, however, a metric exists in the context space so that close-by contexts are expected to provide close-by rewards distributions.   
...
The structure of \textit{online} multiclass classification problems (see  \cite{duda1973pattern} and \cite{freund1999large}) is very close to that of contextual bandit problems. 
...
\begin{itemize}
	...
	\item Second, the knowledge of the universe relies on a \emph{model}. This model embodies both prior information about the structure of the universe and experience-based information. Each new experiment provides additional model information, also called \textit{feedback}, that can be used to update the model.
	\item Third, the construction of the model is constrained by choice optimality.  Contrary to a mere information seeking, each choice relies on a balance between information and final gain. Putative additional information may sometimes be sacrified against putative high reward opportunity.  This is the essence of the exploration/exploitation trade-off, but it also draws a clear connection between bandit problems and the active learning framework \cite{settles2010active}. In that respect, choice may also be referred as \textit{action} for the actual choice (information vs. reward) plays a significant role in the model update.
