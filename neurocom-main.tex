...
The classifier is updated through local measures of $l_t = l(x_t,\tilde{y}_t,f_t,W_t)$  (a loss function) that estimates the correctness of the current response regarding a given misclassification risk minimization constraint.
Combined with a quadratic norm-minimal update condition $\min_W ||W - W_t||^2$, the solution of the optimization problem provides a locally optimal classifier. This local optimization is moreover prone to global improvement, i.e. the total number of updates is shown to be bounded when the input data is linearly-separable.
...
Under  a \emph{relative} scheme (see \cite{crammer2003ultraconservative}), \emph{aka} ``Winner-Takes-All'' (WTA), there is no reference value and the linear score of the class-compliant separatrix $\langle w^y, x\rangle$ is expected to be significantly greater than the other linear scores, i.e.
